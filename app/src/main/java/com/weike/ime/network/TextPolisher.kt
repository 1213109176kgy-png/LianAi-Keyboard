...
interface TextPolisher {
    ...
    suspend fun suggestPolishes(text: String, relation: String): Result<List<String>>
    suspend fun suggestReplies(message: String): Result<List<String>>
...
class MimoTextPolisher(
    ...
    private val replyPromptProvider: () -> String = { AppSettingsRepository.DEFAULT_REPLY_PROMPT },
    private val polishRelationProvider: () -> String = { "通用" },
    private val relationPromptsProvider: () -> Map<String, String> = { AppSettingsRepository.DEFAULT_RELATION_PROMPTS },
    ...
    private fun answerPrompt(): String = replyPromptProvider().trim()
        .takeIf { it.isNotBlank() }
        ?: AppSettingsRepository.DEFAULT_REPLY_PROMPT
    ...
    private fun fastPrompt(
        ...
        return buildString {
            ...
            append(" 对方关系：").append(polishRelationRule()).append("。")
    ...
    override suspend fun suggestPolishes(text: String, relation: String): Result<List<String>> {
        val endpoint = endpointProvider()
        if (!endpoint.isComplete()) return Result.failure(IllegalStateException("请先配置文本模型接口"))
        return try {
            val request = buildRequest(
                endpoint,
                systemPrompt = "你是恋爱沟通助手。用户面对的关系是：$relation。关系专属要求：${relationPromptsProvider()[relation] ?: AppSettingsRepository.DEFAULT_RELATION_PROMPTS.getValue("通用")} 请把用户想说的话改写成3条自然、真诚、语气有差异且可直接发送的话术。只输出JSON字符串数组，必须正好3项，不要解释。",
                userText = text,
                temperature = 0.75,
                stream = false
            )
            val content = client.newCall(request).awaitBody { response ->
                check(response.isSuccessful) { MimoApiConfig.responseError(response, "生成备选话术失败") }
                extractResponseContent(JSONObject(response.body?.string().orEmpty()), endpoint)
            }.trim().removePrefix("```json").removePrefix("```").removeSuffix("```").trim()
            val array = JSONArray(content)
            val options = (0 until array.length()).mapNotNull { array.optString(it).trim().takeIf(String::isNotBlank) }.distinct().take(3)
            check(options.size == 3) { "模型未返回三条有效话术" }
    ...
    override suspend fun suggestReplies(message: String): Result<List<String>> {
        val endpoint = endpointProvider()
        if (!endpoint.isComplete()) return Result.failure(IllegalStateException("请先配置文本模型接口"))
        return try {
            val request = buildRequest(
                endpoint,
                systemPrompt = "${replyPromptProvider()}\n请针对对方消息生成3条语气和表达方式不同、均可直接发送的回复。只输出JSON字符串数组，必须正好3项，不要分析和解释。",
                userText = message,
                temperature = 0.8,
                stream = false
            )
            val content = client.newCall(request).awaitBody { response ->
                check(response.isSuccessful) { MimoApiConfig.responseError(response, "生成推荐回复失败") }
                extractResponseContent(JSONObject(response.body?.string().orEmpty()), endpoint)
            }.trim().removePrefix("```json").removePrefix("```").removeSuffix("```").trim()
            val array = JSONArray(content)
            val replies = (0 until array.length()).mapNotNull { array.optString(it).trim().takeIf(String::isNotBlank) }.distinct().take(3)
            check(replies.size == 3) { "模型未返回三条有效回复" }
    ...
    private fun polishRelationRule(): String = when (polishRelationProvider().trim()) {
        "心动对象" -> "心动对象；语气自然、有吸引力但不过度热情，不擅自确认关系"
        "恋人" -> "恋人；语气亲密温暖，可以自然表达关心，但不编造承诺"
        "朋友" -> "朋友；语气轻松真诚，避免暧昧和越界表达"
        else -> "通用聊天对象；语气自然、礼貌、有分寸"
    }
